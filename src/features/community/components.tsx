...
export function CommunityScreen({
  ...
  return (
    <Screen style={{ maxWidth: 900, gap: 22 }}>
      <View style={c.row}>
        <IconButton
          name="arrow-back"
          label="Back"
        ...
        />
        <Label>ORBIT ROLL · CREW</Label>
        <IconButton
          name="person-outline"
          label="Your pilot profile"
      ...
      <View style={{ gap: 10 }}>
        ...
        <Text style={c.body}>{subtitle}</Text>
      </View>
      {children}
    </Screen>
  );
...
export function OnlineGate({ children }: PropsWithChildren) {
  return supabase ? (
    ...
    <View style={c.card}>
      <Text style={c.heading}>The crew is coming together.</Text>
      <Text style={c.body}>
        Online profiles, rankings, and invites will open once this build is
        connected to Supabase. Your local trails are ready to play.
      </Text>
      <Button
...
export function Notice({
  ...
  return message ? (
    <Text
      ...
      style={[c.body, { color: error ? colors.danger : colors.accent }]}
    >
      {message}
    </Text>
  ) : null;
    ...
    <View style={[c.row, { flexWrap: 'wrap' }]}>
      {[
        ...
        <View key={label} style={{ gap: 6 }}>
          <Text style={c.number}>{value}</Text>
          <Label>{label}</Label>
        </View>
      ))}
    </View>
...
export async function unwrap<T>(
  ...
  if (error)
    throw new Error(
      'Could not load this right now. Check your connection and retry.',
    );
...
export function useResource<T>(key: string, fetcher: () => Promise<T>) {
  ...
  const [state, setState] = useState<{
    ...
    loading: boolean;
    error: string;
  }>({ key, version, data: null, loading: true, error: '' });
  useEffect(() => {
    let alive = true;
    void fetcher().then(
      (data) => {
        if (alive) setState({ key, version, data, loading: false, error: '' });
      },
      () => {
        if (alive)
          setState({
...
export function ResourceStatus({
  ...
  return loading ? (
    ...
    <View style={c.card}>
      ...
      <Button secondary title="Try again" onPress={reload} />
    </View>
  ) : null;
}
