  ...
  if (value)
    await Promise.all(
      Array.from({ length: value.count }, (_, i) =>
        SecureStore.deleteItemAsync(`${key}.${value.version}.${i}`, options),
      ),
    );
}
...
export const authStorage = {
  async getItem(key: string) {
    ...
    const chunks = await Promise.all(
      Array.from({ length: value.count }, (_, i) =>
        SecureStore.getItemAsync(`${key}.${value.version}.${i}`, options),
      ),
    );
  ...
  async setItem(key: string, data: string) {
    ...
    const next = {
      version: randomUUID(),
      count: Math.ceil(characters.length / 350),
    };
    if (next.count < 1 || next.count > 128)
      throw new Error('Session exceeds secure storage capacity');
    await Promise.all(
      Array.from({ length: next.count }, (_, i) =>
        SecureStore.setItemAsync(
          `${key}.${next.version}.${i}`,
          characters.slice(i * 350, (i + 1) * 350).join(''),
          options,
        ),
      ),
    );
